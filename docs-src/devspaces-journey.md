...
<p class="muted">Open OpenShift Dev Spaces from the Developer Sandbox, wire <strong>Continue</strong> to this chart’s LiteLLM Route, and call Hub MCP Actions from the IDE over the public Hub Route.</p>
...
<strong>Continue → LiteLLM</strong>
<span class="desc">Continue is <strong>preinstalled</strong> (Devfile downloads the OpenVSX linux-x64 <code>.vsix</code> into <code>/tmp/continue.vsix</code> and Che Code installs it via <code>DEFAULT_EXTENSIONS</code>). Run <code>wire-continue</code> so Continue v2 writes <code>~/.continue/config.yaml</code> with <code>granite</code> / <code>qwen3</code> → LiteLLM. Select <strong>Granite (LiteLLM)</strong> and chat.</span>
</figcaption>
</figure>

<figure class="journey-slide" data-slide="6" role="group" aria-roledescription="slide" aria-label="7 of 8" hidden>
<img src="{{ '/assets/screenshots/devspaces-continue-mcp-smoke.png' | relative_url }}" alt="Continue with Granite selected beside smoke file showing LiteLLM reply and MCP catalog tool call" />
<figcaption>
<span class="slide-num">Step 07</span>
<strong>MCP from the IDE (Hub Route)</strong>
<span class="desc">From the workspace, call Hub <code>/api/mcp-actions/v1</code> with <code>mcp-token</code> from <code>rhdh-agent-sandbox-secrets</code>. Proven here: LiteLLM reply <em>DevSpaces Continue OK</em> and MCP <code>query-catalog-entities</code> listing sample agents. Wire the same URL into Continue <code>mcpServers</code> (see below).</span>
</figcaption>
</figure>
...
|---|---|---|---|
| Continue chat | LiteLLM Route `/v1` | `rhdh-agent-sandbox-continue` | Yes (Granite / Qwen3 chat) |
| Hub MCP Actions | Hub Route `/api/mcp-actions/v1` | `mcp-token` | Yes (catalog / TechDocs tools) |
| OpenShift / K8s MCP | ClusterIP services | Hub Lightspeed | No — use Hub MCP Chat |

Shared Sandbox models via LiteLLM drop tool calling for Continue. Full K8s tool rounds stay on Hub with `litemaas-qwen` or OpenClaw + LiteMaaS.
